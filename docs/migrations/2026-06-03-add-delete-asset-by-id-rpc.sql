...
declare
  ...
  v_count integer;
begin
  -- array_append (not the || operator): appending an untyped literal via || makes
  -- Postgres resolve it as anyarray||anyarray and try to parse e.g. 'meshes' as an
  -- array literal, raising 22P02 "malformed array literal". array_append binds the
  -- literal to anyelement unambiguously.
